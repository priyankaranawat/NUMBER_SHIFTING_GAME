...
void game(char player_name[50])                                            //GENERATING RANDOM NUMBERS
{
    ...
    for(i=0;i<4;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            ...
            {
                if(i==3&&j==3){printf("|  %c |",arr[3][3]);break;}
                printf("| %d |",arr[i][j]);
            }
            else
            {
   ...
   i=3;j=3;                         //BECAUSE MATXIX IS 4*4 ,HENCE MAX VALUE OF INDEX IS 4-1=3
   for(k=0;k<moves;)
   {
       ...
       if(ch==72&&i!=3)             //IF UP ARROW KEY IS PRESSED
         {
              swap(i,j,i+1,j,moves-k-1);i++;k++;
         }
       if(ch==75&&j!=3)                     //IF LEFT ARROW KEY IS PRESSED
       {
            swap(i,j+1,i,j,moves-k-1);j++;k++;
       }
